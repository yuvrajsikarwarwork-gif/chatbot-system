 ...
 */
const sendEmailAdapter = async (botId, toEmail, msg) => {
    try {
        // 1. Fetch SMTP Credentials from the Integrations table
        const integrationRes = await (0, db_1.query)(`SELECT credentials FROM integrations WHERE bot_id = $1 AND channel = 'email' AND is_active = true LIMIT 1`, [botId]);
