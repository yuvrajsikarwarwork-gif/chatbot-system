...
const isInputNode = (t: string) => ["input", "menu_button", "menu_list"].includes(t);
    ...
    while (currentNode && loop < 25) {
      ...
      if (currentNode.type === "assign_agent") {
        ...
        activeTimeouts.set(timerKey, setTimeout(async () => {
            ...
            await routeMessage(convId, timeoutMsg, io);
        }, AGENT_TIMEOUT_MS));
      }
      ...
      if (isInputNode(currentNode.type)) break; 
...
export const processIncomingMessage = async (
  ...
  try {
    ...
    const botRes = await query("SELECT id FROM bots WHERE id = $1 AND status = 'active'", [botId]);
    if (!botRes.rows[0]) return;
    ...
    if (!contact) {
        const insertRes = await query(`INSERT INTO contacts (bot_id, platform_user_id, name) VALUES ($1, $2, $3) RETURNING *`, [botId, platformUserId, userName]);
    ...
    if (!conversation) {
        const cInsert = await query(`INSERT INTO conversations (bot_id, contact_id, channel, status, variables) VALUES ($1, $2, $3, 'active', '{}') RETURNING *`, [botId, contact.id, channel]);
        conversation = cInsert.rows[0];
    }

    if (text) {
      await query(`INSERT INTO messages (bot_id, conversation_id, channel, sender, platform_user_id, content) VALUES ($1, $2, $3, 'user', $4, $5)`, [botId, conversation.id, channel, platformUserId, JSON.stringify({ type: "text", text: incomingText })]);
    ...
    if (ESCAPE_KEYWORDS.includes(text)) {
      ...
      await routeMessage(conversation.id, { type: "system", text: "Conversation ended." }, io);
      return; 
    }
    ...
    if (conversation.current_node && !isReset) {
      ...
      if (lastNode && isInputNode(lastNode.type)) {
        ...
        if (!isValid) {
          ...
          if (validationResult.message) await routeMessage(conversation.id, validationResult.message, io);
          ...
          if (validationResult.step) {
             ...
             if (targetNode) {
                 ...
                 for (const action of actions) await routeMessage(conversation.id, action, io);
             }
          }
          return;
        }
    ...
    if (!currentNode || isReset) {
        ...
        if (currentNode) await query("UPDATE conversations SET current_node = NULL, variables = '{}', status = 'active', retry_count = 0 WHERE id = $1", [conversation.id]);
    }
